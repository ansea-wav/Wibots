...
const getPlans = (t: (k: string) => string) => [
  {
    ...
    originalPrice: null,
    discountTag: null,
    features: ['1 Auto Responder', 'Tanpa Unggah Berkas', 'API Terbatas', '1 Grup Terhubung'],
    icon: 'spa',
  },
  {
    id: 'basic',
    ...
    discountTag: '75% OFF',
    features: ['30 Auto Responder', 'AI Zeina X', 'Panel Petrodactyl'],
    icon: 'grade',
  },
  {
    id: 'standard',
    ...
    discountTag: '80% OFF',
    features: ['50 Auto Responder', 'AI Zeina Pro', 'Panel Petrodactyl', 'Custom Servertag / Group Tag'],
    icon: 'stars',
    popular: true,
  },
  {
    ...
    features: ['150 Auto Responder', 'AI Zeina Pro', 'Panel Hosting', 'Custom API', 'Special Domain', 'SFTP', 'Custom Servertag / Group Tag'],
...
export default function SubscriptionApp({ client }: SubscriptionAppProps) {
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-8 text-left">
      ...
      {/* Current Status Card */}
      <div className="bg-[#fdfcf7] border-2 border-zinc-950 rounded-[2.2rem] p-6 shadow-[4px_4px_0px_#09090b]">
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {getPlans(t).map((plan) => (
            <div
              key={plan.id}
              className="bg-[#fdfcf7] border border-zinc-950 rounded-[2.2rem] p-6 shadow-[4px_4px_0px_#09090b] flex flex-col justify-between min-h-[24rem] relative"
            >
              {plan.popular && (
                <div className="absolute -top-3 right-6 px-3 py-1 bg-zinc-950 text-white border border-zinc-950 rounded-full text-[9px] font-black uppercase tracking-wider shadow-[2px_2px_0px_#000000]">
                  Most Popular
                </div>
              )}
                ...
                <div className="flex items-center justify-between border-b border-zinc-200/60 pb-3">
                  ...
                  <div className="text-right">
                    <div className="flex items-center gap-1.5 justify-end">
                      {plan.originalPrice && (
                        <span className="text-[10px] text-zinc-400 line-through font-bold">{plan.originalPrice}</span>
                      )}
                      {plan.discountTag && (
                        <span className="bg-rose-500 text-white border border-zinc-950 px-1.5 py-0.5 rounded text-[8px] font-black">{plan.discountTag}</span>
                      )}
                    </div>
                    <div className="text-lg font-black text-zinc-950 mt-0.5">{plan.price}</div>
                    <div className="text-[8px] text-zinc-400 font-bold uppercase tracking-wider">/ month</div>
                  </div>
                </div>
        ...
        {!showCalc ? (
          <div className="text-center py-2">
            ...
            <button 
              ...
              className="inline-flex items-center gap-2 mt-4 px-6 py-2.5 rounded-full text-xs font-black bg-zinc-950 hover:bg-zinc-900 text-white transition-all shadow-[3px_3px_0px_#000000] border border-zinc-950 cursor-pointer"
            >
              ⚙ Custom Calculator
            </button>
          </div>
          ...
          <div className="space-y-5">
            <div className="flex justify-between items-center pb-2 border-b border-zinc-200/50">
              <h3 className="font-black text-zinc-950 text-sm">Adjust Custom Package</h3>
              <button 
            ...
            <div className="space-y-4">
              <div>
                <div className="flex justify-between text-xs mb-1.5 font-bold">
                  ...
                  <span className="text-zinc-950">{responSlots} Slots</span>
                </div>
                <input 
                  type="range" 
                  min="25" 
              ...
              <div>
                <div className="flex justify-between text-xs mb-1.5 font-bold">
                  ...
                  <span className="text-zinc-950">{groupSlots} Groups</span>
                </div>
                <input 
                  type="range" 
                  min="1" 
